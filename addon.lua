...
function UI:Construct()
    ...
    hint.Text = UIUtils:Clean("右側ctrl關閉")
    ...
    if State.Mode == "simple" then
        self:Label("簡單版設定")
        self:Toggle("開啟自瞄", nil, function(v) State.Config.Aimbot.Enabled = v end)
        self:Slider("fov大小", 50, 600, 150, nil, function(v) State.Config.Aimbot.FOV = v end)
        self:Label("進階")
        self:Toggle("隊伍確認", nil, function(v) State.Config.Aimbot.TeamCheck = v end)
        self:Toggle("自動扳機", nil, function(v) State.Config.Triggerbot.Enabled = v end)
    else
        self:Label("自瞄設定")
        self:Toggle("開啟自瞄", nil, function(v) State.Config.Aimbot.Enabled = v end)
        self:Dropdown("身體", DropdownList, function(v) State.Config.Aimbot.Part = v end)
        self:Slider("fov大小", 50, 600, 150, nil, function(v) State.Config.Aimbot.FOV = v end)
        ...
        self:Toggle("牆壁檢測", "檢測視角內是否有牆", function(v) State.Config.Aimbot.WallCheck = v end)
        self:Toggle("隊伍確認", nil, function(v) State.Config.Aimbot.TeamCheck = v end)
        self:Toggle("多項自瞄", "如果有頭就鎖頭沒有就鎖身", function(v) State.Config.Aimbot.Multipoint = v end)
        
        self:Label("高級版")
        self:Toggle("自動扳機", "自動開火", function(v) State.Config.Triggerbot.Enabled = v end)
    end
